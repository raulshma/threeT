...
namespace ThreeTee.Api.Controllers
{
    ...
    public class ModuleController : ControllerBase
    {
        ...
        //[Route("/api/Module/GetByProject")]
        //[HttpGet("{projectId}")]
        [HttpGet("GetByProject", Name = nameof(GetByProject))]
        public async Task<IResult> GetByProject(Guid projectId)
        {
            var item = await _moduleService.GetAllByProjectID(projectId);
            if (item == null) return TypedResults.NotFound();
            return TypedResults.Ok(item);
        }
    }
